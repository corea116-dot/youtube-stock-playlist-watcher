...
8. 새 영상마다 자막을 가져옵니다.
9. 자막이 있으면 자막 기반으로 Gemini 분석을 실행합니다.
10. 자막이 없으면 Gemini YouTube URL 직접 분석을 먼저 시도합니다.
11. URL 직접 분석도 실패하면 제목/설명/태그/댓글 기반 제한 분석을 시도합니다.
12. 댓글도 없거나 실패하면 제목/설명/태그 기반 제한 분석을 사용합니다.
13. 분석에 성공한 결과가 있으면 이메일을 보냅니다.
14. 이메일 발송이 성공한 경우에만 해당 `video_id`를 처리 완료로 기록합니다.
