...
5. 수동 실행에서 `force_reprocess=true`를 선택하면 이미 처리한 영상이어도 최근 영상 1개를 다시 분석해 이메일 발송 테스트를 합니다.
6. 새 영상이 없고 `force_reprocess=false`이면 이메일을 보내지 않은 이유를 로그로 남기고 종료합니다.
7. 새 영상마다 자막을 가져옵니다.
8. 자막이 있으면 자막 기반으로 Gemini 분석을 실행합니다.
9. 자막이 없으면 Gemini YouTube URL 직접 분석을 먼저 시도합니다.
10. URL 직접 분석도 실패하면 제목/설명/태그/댓글 기반 제한 분석을 시도합니다.
11. 댓글도 없거나 실패하면 제목/설명/태그 기반 제한 분석을 사용합니다.
12. 분석에 성공한 결과가 있으면 이메일을 보냅니다.
13. 이메일 발송이 성공한 경우에만 해당 `video_id`를 처리 완료로 기록합니다.
...
4. `force_reprocess` 값을 선택합니다.
   - 평소 테스트: `false`
   - 이메일 발송까지 강제로 확인하고 싶을 때: `true`
5. 초록색 실행 항목을 눌러 로그를 확인합니다.

`force_reprocess=true`는 이미 처리한 영상이어도 최근 영상 1개를 다시 분석해서 이메일을 보내는 수동 테스트용 옵션입니다. GitHub Actions가 성공으로 끝났는데 이메일이 오지 않을 때, “이메일 발송 설정이 실제로 맞는지” 확인하는 데 사용합니다.

평소 자동 실행에서는 이 값이 기본적으로 `false`입니다. 그래서 이미 처리한 영상에 대해 중복 이메일을 계속 보내지 않습니다.

로그에는 Secret 값이 출력되지 않도록 했습니다. 대신 설정 로드 완료, 감시하는 재생목록 개수, 가져온 영상 개수, 이미 처리된 영상 개수, 새로 처리할 영상 개수, 이메일을 보냈는지 또는 보내지 않은 이유가 보이게 됩니다.
