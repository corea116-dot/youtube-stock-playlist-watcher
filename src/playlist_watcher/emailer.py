...
def _build_plain_text_body(analyses: list[dict]) -> str:
    ...
    if _has_test_mode(analyses):
        sections.extend(
            [
                "테스트 모드로 실행되었습니다. 이미 처리된 영상도 다시 분석했을 수 있습니다.",
                "",
            ]
        )
...
def _build_html_body(analyses: list[dict]) -> str:
    ...
    test_mode_notice = (
        "<p><strong>테스트 모드:</strong> 테스트 모드로 실행되었습니다. "
        "이미 처리된 영상도 다시 분석했을 수 있습니다.</p>"
        if _has_test_mode(analyses)
        else ""
    )
...
def _has_test_mode(analyses: list[dict]) -> bool:
    ...
    return any(
        _string_value(analysis.get("test_mode")) in {"today", "latest_one"}
